...
Float_t LHE_weights_scale_wgt[10];
    ...
    TH1F *hJetqqbb_pz = new TH1F("hJetqqbb_pz", "", 30, 0., 600.);
...
//		genweight/=countLHEPdf->GetBinContent( countLHEPdf->FindBin(100) )/xsec[file_tag]; 
...
//		}
	...
	//	genweight*=LHE_weights_scale_wgt[scale_counter];
////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////
		...
		for (int i=0;i<loopJet_max;i++){
			if ((i==btag_max1_number)||(i==btag_max2_number)||(i==pt_max1_number)||(i==pt_max2_number)||(Jet.id[i]<=2)||(Jet.puId[i]==0)) continue;
			Jet5pt=Jet.pt[i];
			break;
		}
	...
	/*			if (set_type==0) if (bdt[0]>cats[NCATS-1]) hMbb_regVBF_fsr_high_cat->Fill(Mbb_reg_fsr,genweight);
				...
				hJet1q_axis2->Fill(TMath::Exp((-1)*Jet.axis2[pt_max1_number]),genweight);
				hJet1q_mult->Fill(Jet.mult[pt_max1_number],genweight);
				hJet1q_leadTrackPt->Fill(Jet.leadTrackPt[pt_max1_number],genweight);
				...
				hJet2q_axis2->Fill(TMath::Exp((-1)*Jet.axis2[pt_max2_number]),genweight);
				hJet2q_mult->Fill(Jet.mult[pt_max2_number],genweight);
				hJet2q_leadTrackPt->Fill(Jet.leadTrackPt[pt_max2_number],genweight);
				...
				hJet1b_pt->Fill(Jet.pt[btag_max1_number],genweight);
				hJet1b_eta->Fill(Jet.eta[btag_max1_number],genweight);
				hJet2b_pt->Fill(Jet.pt[btag_max2_number],genweight);
				hJet2b_eta->Fill(Jet.eta[btag_max2_number],genweight);
...
//			hqgl2_VBF->Fill(Jet.qgl2_VBF[pt_max2_number],genweight);
			...
			hbtag_log->Fill(-1.*TMath::Log(1.-Jet.btag[btag_max1_number]),genweight);
			hbtag2_log->Fill(-1.*TMath::Log(1.-Jet.btag[btag_max2_number]),genweight);
			hrho->Fill(rho,genweight);
			if ((Mbb_reg_fsr>=80)&&(Mbb_reg_fsr<=200))  hprof->Fill(bdt,Mbb_reg_fsr,genweight);
//			if ((bdt[0]>cats[NCATS-1])) hMbbcat3->Fill(Mbb_reg_fsr,genweight);
//			if (bdt[0]>cats[NCATS-2]) hMbbcat2->Fill(Mbb_reg_fsr,genweight);
//			if (bdt[0]>cats[NCATS-3]) hMbbcat1->Fill(Mbb_reg_fsr,genweight);
...
//			hvLeptons_sig->Fill(TMath::Abs(vLeptons_dz[0])/vLeptons_edz[0],genweight);
	...
	float sum_events_er_plus=0;
	float sum_events_er_minus=0;
	float sum_events_er=0;
	float sum_events_er_true=0;
	float sum_events_er_true_80_200=0;
	for (int i=0;i<NCATS;i++){
		sum_events_er_plus+=events_in_cats_er_plus[i];
		sum_events_er_minus+=events_in_cats_er_minus[i];
		events_in_cats_er[i]=TMath::Sqrt(events_in_cats_er_plus[i])/events_in_cats_er_minus[i];
	}
	sum_events_er=TMath::Sqrt(sum_events_er_plus)/sum_events_er_minus;


	out<<"for 2.32 fb^-1:"<<endl;
	float sum_events=0;
	float sum_events_80_200=0;
	for (int i=0;i<NCATS;i++){
		if (data!=1) events_in_cats[i]*=2320.;
		if (data!=1) events_in_cats_80_200[i]*=2320.;
		events_in_cats_er_true[i]=TMath::Sqrt(events_in_cats_er_true[i]);
		events_in_cats_er_true_80_200[i]=TMath::Sqrt(events_in_cats_er_true_80_200[i]);
		if (data!=1) events_in_cats_er_true[i]*=2320./(events_generated/xsec[file_tag]);
		if (data!=1) events_in_cats_er_true_80_200[i]*=2320./(events_generated/xsec[file_tag]);
		sum_events+=events_in_cats[i];
		sum_events_80_200+=events_in_cats_80_200[i];
		out<<"events in cat"<<i << " : "<<events_in_cats[i] << "  ,  ";
	}
	out<<endl;
	out<<file_tag<<"\t";
	for (int i=0;i<NCATS;i++)
		out<<std::fixed<<std::setprecision(2)<<events_in_cats[i] << "\t";
	out<<std::fixed<<std::setprecision(2)<<sum_events<<"\t";
	out<<endl;
	out<<file_tag<<"\t";
	out<<std::fixed<<std::setprecision(2)<<sum_events;
	out<<endl;
	...
	out<<file_tag<<"\t";
	for (int i=0;i<NCATS;i++)
		out<<std::fixed<<std::setprecision(2)<<events_in_cats_er_true[i] << "\t";
	out<<std::fixed<<std::setprecision(2)<<sum_events_er_true<<"\t";
	out<<endl;
	out<<"MC error in 80-200 window"<<endl;
	out<<file_tag<<"\t";
	for (int i=0;i<NCATS;i++)
		out<<std::fixed<<std::setprecision(2)<<events_in_cats_er_true_80_200[i] << "\t";
	out<<std::fixed<<std::setprecision(2)<<sum_events_er_true_80_200<<"\t";
	out<<endl;
	out<<"positive + negative"<<endl;
	out<<file_tag<<"\t";
	for (int i=0;i<NCATS;i++)
		out<<std::fixed<<std::setprecision(2)<<events_in_cats_er_plus[i] << "\t";
	out<<std::fixed<<std::setprecision(2)<<sum_events_er_plus<<"\t";
	out<<endl;
	out<<"positive - negative"<<endl;
	out<<file_tag<<"\t";
	for (int i=0;i<NCATS;i++)
		out<<std::fixed<<std::setprecision(2)<<events_in_cats_er_minus[i] << "\t";
	out<<std::fixed<<std::setprecision(2)<<sum_events_er_minus<<"\t";
	out<<endl;
	out<<file_tag<<"\t";
	for (int i=0;i<NCATS;i++)
		out<<std::fixed<<std::setprecision(2)<<events_in_cats_80_200[i] << "\t";
	out<<std::fixed<<std::setprecision(2)<<sum_events_80_200<<"\t";
	out<<endl;
	out.close();
