int preselection_single(Int_t nJets, Float_t Jet_pt[300], Float_t Jet_eta[300], Float_t Jet_phi[300], Float_t Jet_mass[300], Float_t Jet_btagCSV[300], Int_t Jet_id[300], Int_t& btag_max1_number, Int_t& btag_max2_number, Int_t& pt_max1_number, Int_t& pt_max2_number, Float_t trigger, TLorentzVector& Bjet1,TLorentzVector& Bjet2, TLorentzVector& Qjet1, TLorentzVector& Qjet2,TLorentzVector& qq,Int_t &cut_count){
	...
	if (nJets<4) return -1;
/*
	if (!(Jet_pt[0]>(92.+10.))) not_pass= -2;
	if (!(Jet_pt[1]>(76.+10.))) not_pass = -3;
	if (!(Jet_pt[2]>(64.+10.))) not_pass=-4;
	if (!(Jet_pt[3]>(30.+10.)))  not_pass = -5;*/
	if (!(Jet_pt[0]>92.)){ not_pass= -2; cut_count++;}
	if (!(Jet_pt[1]>76.)) {not_pass = -3;cut_count++;}
	if (!(Jet_pt[2]>64.)) {not_pass=-4; cut_count++;}
	if (!(Jet_pt[3]>30.)) { not_pass = -5; cut_count++;}

		int loopJet_min = 4;

		Double_t btag_max = 0.0;
		...
		if (!(Mqq>460)) {not_pass=-6; cut_count++;}
		if (!(qqDeltaEta>4.1)) {not_pass=-7; cut_count++;}
