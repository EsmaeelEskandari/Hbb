...
//	if (type_int==0) NCATS_single= 4;
	...
	if (type_int==1) {
		cats={-1.,-0.5,0.31,0.59,0.67,0.7325};
	}
	if (type_int==0) {
		cats={-1.,0.12,0.44,0.57,0.6725};
	}
...
//	TFile *file_down =  new TFile("/shome/nchernya/Hbb/slim_trees/SysUnc/JECR/new/main_mva_v14_output_"+s_names[0]+"_76x_v20_6"+type+ToLower(jec)+corType[i]+"_v20.root");
	...
	TH1F *hist_mbb_JEdown= new TH1F("hist_mbb_"+jec+"_"+corType[i],"",50,50,200);
...
//	sprintf(cmd2,"weight/995825.*((BDT_VBF_\%s_\%s>\%.2f) && (BDT_VBF_\%s_\%s<=\%.2f))",jec.Data(),corType[i].Data(),cats[cat_num],jec.Data(),corType[i].Data(),cats[cat_num+1]);	 tree_bdt_JEdown->Draw(cmd3,cmd2);
	sprintf(cmd2,"weight*((BDT_VBF_\%s_\%s>\%.2f) && (BDT_VBF_\%s_\%s<=\%.2f))",jec.Data(),corType[i].Data(),cats[cat_num],jec.Data(),corType[i].Data(),cats[cat_num+1]);	 tree_bdt_JEdown->Draw(cmd3,cmd2);
...
//workspaces
	...
	RooRealVar yVBF(xVar,xVar,hist_mbb_JEdown->Integral());
	...
	RooRealVar s(tmp.Format("sigma_CAT%d_%s",cat_num,corType[i].Data()),tmp.Format("sigma_CAT%d_%s",cat_num,corType[i].Data()),12,3,30);
	RooRealVar width(tmp.Format("fwhm_CAT%d_%s",cat_num,corType[i].Data()),tmp.Format("fwhm_CAT%d_%s",cat_num,corType[i].Data()),25,0,100);
...
//	TH1F *try2 =  (TH1F*)hist_mbb_JEdown->Clone("try2");
//	try2->Scale(2.);
	RooDataHist hScale("tmp","tmp",RooArgList(x),hist_mbb_JEdown);
//	RooDataHist hScale("tmp","tmp",RooArgList(x),try2);
...
// CMS info
	...
	c_roo->Print("plots/shape/v14_vbf"+type+"_"+jec+"_mbb_"+cats_names[cat_num]+corType[i]+"_2.png");
...
w.writeToFile("plots/shape/workspace"+type+"_"+jec+"_mbb_2.root");
	return 0;
