...
//	TFile *file_down =  new TFile("inputs/main_mva_v14_output_"+s_names[0]+"_76x_v20_6"+type+ToLower(jec)+corType[i]+"_v20.root");
	...
	hist_mbb_scaled->Scale(lumi*xsec[0]/events_generated);
///////// GF
	
	TCanvas *cgf = new TCanvas();
	TFile *file_gf =  new TFile("/mnt/t3nfs01/data01/shome/nchernya/Hbb/tmva/main_mva/SysUnc/v21/main_mva_v21_"+s_names[1]+type+ToLower(jec)+corType[i]+"_v21.root");
	TH1F*	CountPosWeight_gf = (TH1F*)file_gf->Get("CountPosWeight");
	TH1F*	CountNegWeight_gf =(TH1F*)file_gf->Get("CountNegWeight");
	float events_generated_gf = CountPosWeight_gf->GetEntries()-CountNegWeight_gf->GetEntries();	
	TTree *tree_bdt_JEdown_gf = (TTree*)file_gf->Get(ToLower(jec)+corType[i]);
	TH1F *hist_bdt_JEdown_gf= new TH1F("hist_bdt_"+jec+"_"+corType[i]+"_gf","",NCATS,cats);
	hist_bdt_JEdown_gf->GetXaxis()->SetTitle("BDT output"); 
	...
	sprintf(cmd,"\%s>>\%s",("BDT_VBF_"+jec+"_"+corType[i]).Data(),("hist_bdt_"+jec+"_"+corType[i]+"_gf").Data());	
	sprintf(cmd3,"\%s>>\%s",("Mbb_VBF_"+jec+"_"+corType[i]).Data(),("hist_mbb_"+jec+"_"+corType[i]+"_gf").Data());
	sprintf(cmd2,"trigWeight*puWeight*bTagWeight*((BDT_VBF_\%s_\%s>\%.2f) && (BDT_VBF_\%s_\%s<=\%.2f))",jec.Data(),corType[i].Data(),cats[cat_num],jec.Data(),corType[i].Data(),cats[cat_num+1]);
