...
//	int type_int; //double  =0; single=1
	...
	const int NUNC = 9;
	TString type_unc[20];
	TString type_unc_str[20] = {"JES", "LF", "HF", "LFStats1", "LFStats2", "HFStats1", "HFStats2", "cErr1", "cErr2"};
	TString up_str = "Up";
	TString down_str = "Down";
	for (int i=0;i<NUNC;i++){
		type_unc[i] = "bTagWeight_";
		type_unc[i].Append(type_unc_str[i]);
	}
	TString type;
...
//	TString s_names[num_ss] = {"vbf_76"};
	...
	sprintf(cmd2,"trigWeight*puWeight*\%s\%s",(type_unc[type_unc_int].Data()),("Down"));	
	tree_bdt_JEnom->Draw(cmd,cmd2);
	tree_bdt_JEnom->Draw(cmd3,cmd2);
	...
	sprintf(cmd2,"trigWeight*puWeight*\%s\%s",(type_unc[type_unc_int]).Data(),"Up");	
	tree_bdt_JEnom->Draw(cmd,cmd2);
	tree_bdt_JEnom->Draw(cmd3,cmd2);
	...
	ofstream out;
	out.open("output_txt/v21/datacards_sysUnc_"+s_names[sig_num]+type+type_unc[type_unc_int]+".txt");
...
//	if (type_int==1)  frame2->SetMaximum(0.005);
   frame2->SetStats(0);
	...
	pave.AddText(tmp.Format("\%s\%s","bTagWeight ",type_unc_str[type_unc_int].Data()));
	pave.AddText(tmp);
	pave.Draw("same");
...
////////
	...
	c1->Print("plots/v21/v21_"+s_names[sig_num]+type+"_btagWeight"+type_unc_str[type_unc_int]+".png");
