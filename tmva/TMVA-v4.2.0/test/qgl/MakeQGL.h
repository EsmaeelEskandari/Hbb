...
public :
   ...
   MakeQGL(TTree *tree=0, TString filename="");
   virtual ~MakeQGL();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop(TString input_filename="",TString output_dir="",int set_type=0);
...
#ifdef MakeQGL_cxx
MakeQGL::MakeQGL(TTree *tree, TString filename) : fChain(0) 
{	   
	if (tree == 0) {
			file_name = filename;
...
Int_t MakeQGL::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t MakeQGL::LoadTree(Long64_t entry)
{
...
void MakeQGL::Init(TTree *tree)
{
   // The Init() function is called when the selector needs to initialize
   // a new tree or chain. Typically here the MakeQGL addresses and MakeQGL
   ...
   // Set MakeQGL addresses and MakeQGL pointers
   if (!tree) return;
   fChain = tree;
   fCurrent = -1;
...
void MakeQGL::Show(Long64_t entry)
{
...
Int_t MakeQGL::Cut(Long64_t entry)
{
