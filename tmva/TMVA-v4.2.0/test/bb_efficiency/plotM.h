...
#ifdef plotM_cxx
plotM::plotM(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../b_likelihood_powheg_6var_bbqq_125_08.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../b_likelihood_powheg_6var_bbqq_125_08.root");
      }
...
Int_t plotM::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t plotM::LoadTree(Long64_t entry)
{
...
void plotM::Show(Long64_t entry)
{
...
Int_t plotM::Cut(Long64_t entry)
{
