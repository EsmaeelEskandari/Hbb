#define plotMqlike_cxx
#include "plotMqlike.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
  ...
  float cont_1q=0,cont_2q=0, cont_1q_=0, cont_2q_=0;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      JetList jetList_CSV, jetList_bl, jetList_ql;
		...
		if (!((Jet_pt[0]>92.)&&(Jet_pt[1]>76.)&&(Jet_pt[2]>64.)&&(Jet_pt[3]>30.))) { cout<< " 1  , "<<jentry<<endl; continue; }
		...
		if (!((btag_max1_number>=0)&&(btag_max2_number>=0))) { cout<< " 2  , "<<jentry<<endl; continue;}
		...
		if (!((pt_max1_number>=0)&&(pt_max2_number>=0))){ cout<< " 3  , "<<jentry<<endl;  continue;}
		...
		if (!((Mqq>200)&&(qqDeltaEta>1.2)&&(bbDeltaPhi<2.4))) { cout<< " 4  , "<< Mqq<<"   "<< qqDeltaEta<<"  "<< bbDeltaPhi <<"  " <<jentry<<endl; continue;}

		if (HLT_BIT_HLT_QuadPFJet_DoubleBTagCSV_VBF_Mqq200_v!=1) { cout<< " 5  , "<<jentry<<endl; continue;}
