...
// Thu Aug 13 10:53:49 2015 by ROOT version 5.34/32
...
#ifdef plotMqlike_cxx
plotMqlike::plotMqlike(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../b_likelihood_powheg_6var_bbqq5eta_125_08_qlike.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../b_likelihood_powheg_6var_bbqq5eta_125_08_qlike.root");
      }
...
plotMqlike::~plotMqlike()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t plotMqlike::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t plotMqlike::LoadTree(Long64_t entry)
{
...
void plotMqlike::Init(TTree *tree)
{
...
void plotMqlike::Show(Long64_t entry)
{
...
Int_t plotMqlike::Cut(Long64_t entry)
{
