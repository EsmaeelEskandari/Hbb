...
//	TString n_1_str_end[2] = {"","_n_1_tighter"};
	...
	TString n_1_str_end[2] = {"","_btagW_puid"};
//	TString n_1_str_end[2] = {"","_n_4"};
...
/*
	...
	//	func->SetParameters(3.,0.1,92.);
		...
		float par1_up, par1_down, par2_up, par2_down, par3_up, par3_down;
		par1 = .7; par2 = 85.; par3 = 15.;
		par2_down = 7.; par2_up = 29.;
		par3_down = 0.8; par3_up =  1.2;
		func_r.SetParameters(par1,par2,par3);// double jet 1
		func_r.SetParLimits(2,par2_down,par2_up); //double 1 jet
		func_r.SetParLimits(0,par3_down,par3_up); //double 1 jet
	...
	//		hist_Ratio_down->Fit(&func_r_down);
		...
		func_r_up.SetParameters(func_r.GetParameter(0)*1.1,func_r.GetParameter(0)*1.1,func_r.GetParameter(0)*1.1);// double jet 1
		func_r_up.SetParLimits(2,func_r.GetParameter(2)*1.05,func_r.GetParameter(2)*1.2); //double 1 jet
		func_r_up.SetParLimits(0,func_r.GetParameter(0)*1.05,func_r.GetParameter(0)*1.2); //double 1 jet
	//	func_r_up.SetParLimits(0,par3_down,par3_up); //double 1 jet
		...
		func_r_down.SetParameters(func_r.GetParameter(0)*0.9,func_r.GetParameter(0)*0.9,func_r.GetParameter(0)*.9);// double jet 1
		func_r_down.SetParLimits(2,func_r.GetParameter(2)*0.8,func_r.GetParameter(2)*0.95); //double 1 jet
		func_r_down.SetParLimits(0,func_r.GetParameter(0)*0.8,func_r.GetParameter(0)*0.95); //double 1 jet
		if ((current_hist==histFit)) {
	//		hist_Ratio->Fit(&func_r);
			hist_Ratio_up->Fit(&func_r_up);
			hist_Ratio_down->Fit(&func_r_down);
...
//		par3_up = par3_up*1.2;
//		par2_up = par2_up*1.2;
//		func_r_up.SetParameters(par1,par2,par3);// double jet 1
//		func_r_up.SetParLimits(2,par2_down,par2_up); //double 1 jet
//		func_r_up.SetParLimits(0,par3_down,par3_up); //double 1 jet
//		par2_down = par2_down*0.8; 
//		par3_down = par3_down*0.8;
//		func_r_down.SetParameters(par1,par2,par3);// double jet 1
//		func_r_down.SetParLimits(2,par2_down,par2_up); //double 1 jet
//		func_r_down.SetParLimits(0,par3_down,par3_up); //double 1 jet
//		func_r.SetParameters(0.7,70.,14);// double jet 2
//		func_r.SetParLimits(2,10.,30); //double 2 jet
//		func_r.SetParameters(0.7,41.,30);// double jet 3
//		func_r.SetParLimits(2,10.,60); //double 3 jet
//		func_r.SetParameters(0.7,18.,11);// double jet 4
//		func_r.SetParLimits(2,5.,30); //double 4 jet
//		func_r.SetParameters(0.72,151,44); //double mqq 
//		func_r.SetParameters(0.8,1.1,3.7);//double csv1
//		if (current_hist==0) hist_Ratio->Fit(func_r,"M");
...
//	frame2->GetXaxis()->SetNdivisions(0);
  ...
  //  	frame2->GetXaxis()->SetLabelSize(.12);	
	frame2->Draw();
	TLatex* tex = new TLatex(0.95,0.95,"13 TeV, PU = 20, bx = 25 ns, 2.19 fb^{-1}");
   tex->SetNDC();
	tex->SetTextAlign(35);
   tex->SetTextFont(42);
   ...
   tex2->SetTextSize(0.04);
  	tex2->SetLineWidth(2);
	TLatex* tex_file = new TLatex(0.35,0.95,denominator_trigger_names_str[trigger]);
   tex_file->SetNDC();
...
//	leg->AddEntry(hist_S,tex_s_names[signal_sample_num],"LP");
	...
	//	pad1->RedrawAxis();
		...
		if (current_hist==histFit) c1->Print("plots/"+hist_names[current_hist]+denominator_trigger_names[trigger]+type[set_type]+n_1_str_end[n_1]+".png");
...
//		Float_t x_max2 = hist_Data_csv->GetBinCenter(hist_Data_csv->GetSize())*1.05;
	...
	//		hist_Data_csv->Draw("same");
			...
			c2->Print("plots/CSV1log_map_Data"+denominator_trigger_names[trigger]+type[set_type]+n_1_str_end[n_1]+".png");
		}
		if (current_hist==numArray-2) {
		//	hist_B_csv->Draw("same");
			hist_B_csv->Draw("colz1 texte same");
			c2->Print("plots/CSV1log_map_MC"+denominator_trigger_names[trigger]+type[set_type]+n_1_str_end[n_1]+".png");
		}
	...
	//		hist_Ratio_csv->Draw("Surf1");
  		//	f2d->Draw("surf1 same");
			c2->Print("plots/CSV1log_map_Ratio"+denominator_trigger_names[trigger]+type[set_type]+n_1_str_end[n_1]+".png");		
			c2->Delete();
		}
	}
}
